...
class CanCreateBranch(BasePermission):
    def has_permission(self, request, view):
        ...
        if logged_user not in works_on_list:
            return False
        if WorksOn.objects.get(project__name=repository_name,
                               developer__user__username=logged_user).role == Role.READONLY:
            return False
        return True
...
class CanDeleteRepository(BasePermission):
    def has_permission(self, request, view):
        ...
        return WorksOn.objects.filter(developer__user__username=logged_user,
